    ...
    @endif
        {{
            $attributes->merge($customAttributes)
                ...
                ->class(['flex flex-col sm:table-cell text-center sm:text-start']) // MOBILE VIEW
                ->except(['default','default-styling','default-colors'])
        }}
    >
        <span class="text-gray-500 dark:text-gray-400 font-medium uppercase mb-2 sm:hidden">{{ $column->getTitle() }}</span> {{-- MOBILE VIEW --}}
        {{ $slot }}
</td>
