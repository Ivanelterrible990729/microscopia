                ...
                @foreach ($imageForms as $index => $imageForm)
                    <x-base.table.tr>
                        <x-base.table.td class="text-center" for="#">
                            <x-base.button
                                class="text-blue-500 hover:text-blue-700 underline cursor-pointer border-none shadow-none"
                                type="button"
                        ...
                        <x-base.table.td class="text-center" for="{{ __('Reviewed') }}">
                            {{ $imageForm['reviewed'] ? '✔️' : '✖️' }}
                        </x-base.table.td>
                        <x-base.table.td class="text-center sm:text-start" for="{{ __('Name') }}">
                            {{ $imageForm['name'] }}
                        </x-base.table.td>
                        <x-base.table.td class="text-center sm:text-start" for="{{ __('Description') }}">
                            {{ substr($imageForm['description'], 0, 30) }}...
                        </x-base.table.td>
                        <x-base.table.td class="text-center sm:text-start" for="{{ __('Labels') }}">
                            {{ implode(',', $images[$index]->labels->pluck('name')->toArray()) }}
                        </x-base.table.td>
                    </x-base.table.tr>
                @endforeach
            </x-base.table.tbody>
