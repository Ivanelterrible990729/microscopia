    ...
    <div class="intro-y box mt-5 pt-10 lg:pt-14">
        <div class="flex items-center justify-center">
            ...
            @foreach ($imageForms as $index => $form)
                <x-base.button
                    @class([
                        'intro-y hidden lg:block mx-2 h-10 w-10 rounded-md bg-dark text-white cursor-pointer' => $index != $activeIndex && $form['confirmed'],
                        'intro-y hidden lg:block mx-2 h-10 w-10 rounded-md bg-slate-100 text-slate-500 dark:border-darkmode-400 dark:bg-darkmode-400 cursor-auto' => $index != $activeIndex && !$form['confirmed'],
